...
describe('image', () => {
  it('renders correctly', async () => {
    ...
    expect(wrapper.element.src).toBe('data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7')
  })
  ...
  it('can change the src later', async () => {
    ...
    await flushPromises()
    expect(wrapper.element.src).toBe(`${window.location.origin}${window.location.pathname}`)

    wrapper.setProps({ src: '/img.png' })

    await flushPromises()
    expect(wrapper.element.src).toBe(`${window.location.origin}${window.location.pathname}img.png`)
  })
