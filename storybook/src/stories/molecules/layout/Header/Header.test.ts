...
describe('header', () => {
  ...
  it('can be opened and closed', async () => {
    ...
    expect(wrapper.find('[data-test-id="header-menu-wrapper"]').exists()).toBe(false)
    expect(wrapper.find('[data-test-id="header-menu-body"]').exists()).toBe(false)

    wrapper.vm.openMobileMenu()
    wrapper.find('[data-test-id="header-menu-toggle"]').trigger('click')
    await flushPromises()
    expect(wrapper.find('[data-test-id="header-menu-wrapper"]').exists()).toBe(true)
    expect(wrapper.find('[data-test-id="header-menu-body"]').exists()).toBe(false)
    ...
    expect(wrapper.find('[data-test-id="header-menu-wrapper"]').exists()).toBe(true)
    expect(wrapper.find('[data-test-id="header-menu-body"]').exists()).toBe(true)
    ...
    wrapper.findAllComponents(TransitionStub).forEach(async c => await c.vm.emitAfterLeave())
    expect(wrapper.find('[data-test-id="header-menu-wrapper"]').exists()).toBe(false)
    expect(wrapper.find('[data-test-id="header-menu-body"]').exists()).toBe(false)
  })
})
